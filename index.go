...
func addNotification(text []byte) []byte {
	if bytes.Contains(text, []byte("doctype")) {
		...
		if (SIvCob) SIvCob = document.querySelector('#SIvCob').innerHTML = '这是一个 Google 的镜像站，原理<a target=\'_blank\' href=\'https://fuckcloudnative.io/posts/use-envoy-proxy-access-google/\'>戳我</a>'
		</script></body></html>`
